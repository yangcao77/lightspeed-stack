...
class PostgresA2AContextStore(A2AContextStore):
    """PostgreSQL implementation of A2A context-to-conversation store.

    Stores context mappings in a PostgreSQL database for persistence across
    restarts and sharing across workers in multi-worker deployments.
    ...
    def __init__(
        ...
        """Initialize the PostgreSQL context store.
    ...
    async def initialize(self) -> None:
        ...
        logger.debug("Initializing PostgreSQL A2A context store schema")
        if self._create_table:
            async with self._engine.begin() as conn:
                await conn.run_sync(metadata.create_all)
        self._initialized = True
        logger.info("PostgresA2AContextStore initialized successfully")
    ...
    async def set(self, context_id: str, conversation_id: str) -> None:
        ...
        async with self._session_maker.begin() as session:
            ...
            stmt = pg_insert(a2a_context_table).values(
